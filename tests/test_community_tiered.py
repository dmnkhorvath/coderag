...
def test_fallback_to_connected_components_on_error(analyzer):
    ...
    with patch("networkx.algorithms.community.greedy_modularity_communities", side_effect=RuntimeError("algo failed")):
        result = analyzer.community_detection()
...
def test_fallback_leiden_error_uses_connected_components(analyzer):
    ...
    with patch.object(analyzer, "_leiden_communities", side_effect=ImportError("leidenalg not installed")):
        result = analyzer.community_detection()
        assert isinstance(result, list)
        assert len(result) > 0
...
def test_persist_no_longer_skips_community_detection(analyzer):
    ...
    with patch.object(analyzer, "pagerank", return_value={"node_0": 0.5}):
        with patch.object(analyzer, "community_detection", return_value=[{"node_0", "node_1"}]) as mock_cd:
...
def test_persist_handles_community_timeout(analyzer):
    ...
    with patch.object(analyzer, "pagerank", return_value={"node_0": 0.5}):
        with patch.object(analyzer, "community_detection", side_effect=TimeoutError("timed out")):
            # Should not raise
            analyzer.persist_scores_to_store(store)
...
def test_persist_handles_community_exception(analyzer):
    ...
    with patch.object(analyzer, "pagerank", return_value={"node_0": 0.5}):
        with patch.object(analyzer, "community_detection", side_effect=RuntimeError("unexpected error")):
            analyzer.persist_scores_to_store(store)
